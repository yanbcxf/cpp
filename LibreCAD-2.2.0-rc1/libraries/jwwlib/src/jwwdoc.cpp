...
void CDataSen::Serialize(std::ofstream& ofstr) const
{
    ...
    (/* yangbin */std::ostream&)ofstr	<< (double)m_start.x << (double)m_start.y
...
void CDataTen::Serialize(std::ofstream& ofstr) const
{
    ...
    if( 100 == m_nPenStyle ){
        ofstr << (DWORD )m_nCode;
		(/* yangbin */std::ostream&)ofstr << (double)m_radKaitenKaku;
		(/* yangbin */std::ostream&)ofstr << (double)m_dBairitsu;
    }
}
...
{
    CData::Serialize(ofstr);
	(/* yangbin */std::ostream&)ofstr <<(double)m_DPKijunTen.x <<(double)m_DPKijunTen.y
        <<(double)m_dBairitsuX
        <<(double)m_dBairitsuY
    ...
    *ofs << dw;
    ...
    if( Header.JW_DATA_VERSION >= 300 ){   //Ver.3.00以降
        ...
        *ofs << dw;
		...
		(/* yangbin */std::ostream&)*ofs << db;	//文字列範囲を背景色で描画するときの範囲増寸法
        dw = Header.m_nMojiBG;
        *ofs << dw;
    }
    //複線間隔
