...
CCalcMainIncomeYearGrowth::CCalcMainIncomeYearGrowth(HWND hwnd, int logType)
:CCalculateTask(hwnd, logType)
{
}

CCalcMainIncomeYearGrowth::~CCalcMainIncomeYearGrowth(void)
{
}



void CCalcMainIncomeYearGrowth::Execute1(void* firstArg)
{
	CCalcMainIncomeYearGrowth * sink = (CCalcMainIncomeYearGrowth *)firstArg;
	sink->Execute();
}



void CCalcMainIncomeYearGrowth::Execute()
{
	...
	try
	{
		ss << "select a.code, ( select c.abbreviation from stockinfo c where a.code = c.code ) as abbreviation, "
			...
			<< "colume_2 as income from incomestatement  "
		...
		while (st.fetch())
		{
			...
			{
				m_vec_income.insert(m_vec_income.end(),  -1);
			}
