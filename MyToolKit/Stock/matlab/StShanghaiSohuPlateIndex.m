function handleNumber = StShanghaiSohuPlateIndex( windowNumber, startDate, endDate, graphOrTable, plateCodes )
...
strSql = 'select code, name from souhustockplates where 1 = 1 and Is_Stock = 0 ';
% strSql = strcat(strSql, int2str(plateType));
strSql = strcat(strSql, ' and code in (');

for i = 1: length(plateCodes)
    strSql = strcat(strSql, int2str(plateCodes(i)));
    if i < length(plateCodes)
...
curs = fetch(curs);
plateNames = curs.Data(:,2);
plateCodes = curs.Data(:,1);

strSql = 'select tradedate, point from daily_hushen_plate where tradedate >=''';
...
j = 1;
for i = 1: totalNum
    strSql_ = strcat(strSql, int2str(plateCodes{i, 1}));
    curs = exec(conn, strSql_);
    curs = fetch(curs);
    %     strSql_ = strcat(strSql_, '''');
    if ~strcmp(curs.Data,'No Data')
        areaIndex(j).name = plateNames{i, 1};
        areaIndex(j).code = plateCodes{i, 1};
        
        areaIndex(j).tradedate = cell2mat(curs.Data(:, 1));
        areaIndex(j).point = cell2mat(curs.Data(:, 2))
        j = j + 1;
    end;
