...
IMPLEMENT_DYNCREATE(CMacdListView, CBaseGridCtlView)

CMacdListView::CMacdListView()
{
	/*显示分页栏*/
	m_bPagination = true;
...
BEGIN_MESSAGE_MAP(CMacdListView, CBaseGridCtlView)
	ON_COMMAND(ID_TABLE_ADVANCED_QUERY, &CMacdListView::OnTableAdvancedQuery)
	ON_UPDATE_COMMAND_UI(ID_TABLE_ADVANCED_QUERY, &CMacdListView::OnUpdateTableAdvancedQuery)
	
	ON_COMMAND(ID_TABLE_ADD_TO_SELF_SELECT, &CMacdListView::OnTableAddToSelfSelect)
END_MESSAGE_MAP()


// CCninfoGetLandListView 绘图

void CMacdListView::OnDraw(CDC* pDC)
{
...
#ifdef _DEBUG
void CMacdListView::AssertValid() const
{
	CBaseGridCtlView::AssertValid();
}

#ifndef _WIN32_WCE
void CMacdListView::Dump(CDumpContext& dc) const
{
...
void CMacdListView::OnInitialUpdate()
{
...
void CMacdListView::ProcessXmlWParam(string msgType,string code,string model,\
...
void CMacdListView::RedrawGridCtrl(CCalculateTask * cse)
{
	CCalcMacdQuery * pStockInfoEx = (CCalcMacdQuery *)cse;
...
void CMacdListView::OnTableAdvancedQuery()
{
	// TODO: 在此添加命令处理程序代码

	CSimpleInputBox dlg;
	dlg.m_ComboString.insert(dlg.m_ComboString.end(), "金叉");
	dlg.m_ComboString.insert(dlg.m_ComboString.end(), "死叉");
...
void CMacdListView::OnPageUpdate()
{
	Startup();
}

void CMacdListView::Startup()
{
	CIndividualShareDoc * pDoc = (CIndividualShareDoc *)GetDocument();

	CCalcMacdQuery * cse =  new CCalcMacdQuery(m_hWnd, WM_USER_LOG_1);
	cse->m_request_like = m_strLike;
	cse->m_page_no = m_nPageNo;

	m_MessageCtrl.SetStockRangeForCalculateTask(pDoc, cse);

	cse->ScheduleTask(&CCalcMacdQuery::Execute1);
	m_bCalculating = true;
}

void CMacdListView::OnUpdateTableAdvancedQuery(CCmdUI *pCmdUI)
{
...
void CMacdListView::OnTableAddToSelfSelect()
{
	// TODO: 在此添加命令处理程序代码
...
void CMacdListView::PostGridDblClick(int nRow)
{
	// 弹出 “修改 或者 删除” 对话框
