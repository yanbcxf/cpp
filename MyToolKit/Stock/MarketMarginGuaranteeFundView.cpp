// MarketMarginGuaranteeFundView.cpp : 实现文件
//
...
// CMarketMarginGuaranteeFundView

IMPLEMENT_DYNCREATE(CMarketMarginGuaranteeFundView, CBaseChartCtlView)

CMarketMarginGuaranteeFundView::CMarketMarginGuaranteeFundView()
{
...
BEGIN_MESSAGE_MAP(CMarketMarginGuaranteeFundView, CBaseChartCtlView)
END_MESSAGE_MAP()


// CMarketMarginGuaranteeFundView 绘图

void CMarketMarginGuaranteeFundView::OnDraw(CDC* pDC)
{
...
// CMarketMarginGuaranteeFundView 诊断

#ifdef _DEBUG
void CMarketMarginGuaranteeFundView::AssertValid() const
{
	CBaseChartCtlView::AssertValid();
}

#ifndef _WIN32_WCE
void CMarketMarginGuaranteeFundView::Dump(CDumpContext& dc) const
{
...
// CMarketMarginGuaranteeFundView 消息处理程序



void CMarketMarginGuaranteeFundView::OnInitialUpdate()
{
...
void CMarketMarginGuaranteeFundView::RedrawDmGraph()
{
	CIndividualShareDoc * pDoc = (CIndividualShareDoc *)GetDocument();
	CCalcMarketMarginGuaranteeFund * cse =  new CCalcMarketMarginGuaranteeFund(m_hWnd, WM_USER_LOG_1);

	cse->ScheduleTask(&CCalcMarketMarginGuaranteeFund::Execute1);
}

void CMarketMarginGuaranteeFundView::ProcessXmlWParam(string msgType,string code,string model,\
...
void CMarketMarginGuaranteeFundView::RedrawDmGraph(CCalculateTask * cse)
{
	CIndividualShareDoc * pDoc = (CIndividualShareDoc *)GetDocument();
	...
	CCalcMarketMarginGuaranteeFund * pCse =  (CCalcMarketMarginGuaranteeFund *)cse;
	...
	if(pCse->m_vec_begin_date.size()>0)
	{
		...
		{
			COleDateTime odt ;
			double yy = pCse->m_vec_begin_index[nStart + i] * 10;
			odt.ParseDateTime(pCse->m_vec_begin_date[nStart + i].c_str());
			pSeries->AddPoint(odt.m_dt, yy);
			if(yy > ymax)  ymax = yy;

			odt.ParseDateTime(pCse->m_vec_end_date[nStart + i].c_str());
			yy = pCse->m_vec_end_index[nStart + i] * 10;
			pSeries->AddPoint(odt.m_dt, yy);
			if(yy > ymax)  ymax = yy;
		}
