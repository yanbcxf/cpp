...
CCalcMarketExchangeFund::CCalcMarketExchangeFund(HWND hwnd, int logType)
:CCalculateTask(hwnd, logType)
{
}

CCalcMarketExchangeFund::~CCalcMarketExchangeFund(void)
{
}
...
void CCalcMarketExchangeFund::Execute1(void* firstArg)
{
	CCalcMarketExchangeFund * sink = (CCalcMarketExchangeFund *)firstArg;
	sink->Execute();
}



void CCalcMarketExchangeFund::Execute()
{
	...
	try{
		...
		ss << "select date_format(begin_date, '%Y-%m-%d') as begin_date, date_format(end_date, '%Y-%m-%d') as end_date, "
			...
			<< " from fundinsettlement a where fund_type='证券交易' order by end_date ";
		...
		ss.str("");
		ss << "CCalcMarketExchangeFund 统计完成";
