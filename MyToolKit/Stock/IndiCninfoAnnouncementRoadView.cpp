...
IMPLEMENT_DYNCREATE(CIndiCninfoAnnouncementRoadView, CBaseMessageFormView)

CIndiCninfoAnnouncementRoadView::CIndiCninfoAnnouncementRoadView()
{
...
BEGIN_MESSAGE_MAP(CIndiCninfoAnnouncementRoadView, CBaseMessageFormView)
...
void CIndiCninfoAnnouncementRoadView::OnDraw(CDC* pDC)
{
...
#ifdef _DEBUG
void CIndiCninfoAnnouncementRoadView::AssertValid() const
{
	CBaseMessageFormView::AssertValid();
}

#ifndef _WIN32_WCE
void CIndiCninfoAnnouncementRoadView::Dump(CDumpContext& dc) const
{
...
void CIndiCninfoAnnouncementRoadView::OnInitialUpdate()
{
...
void CIndiCninfoAnnouncementRoadView::RedrawDmGraph()
{
	CIndividualShareDoc * pDoc = (CIndividualShareDoc *)GetDocument();
	CCalcCninfoAnnouceRoadMap * cse =  new CCalcCninfoAnnouceRoadMap(m_hWnd, WM_USER_LOG_1);
	cse->m_vec_request_code.push_back( pDoc->m_nCode );
	cse->m_str_request_zb_code = pDoc->m_strInput;

	cse->ScheduleTask(&CCalcCninfoAnnouceRoadMap::Execute1);
}



string CIndiCninfoAnnouncementRoadView::ExtractKeyword(string title)
{
	string strTip1 = "";
	...
	if(title.find("中止")!=string::npos )	return "中止";
	if(title.find("终止")!=string::npos )	return "终止";
...
void CIndiCninfoAnnouncementRoadView::RedrawChart(CCalculateTask * cse)
{
	...
	for(int nRow = 0 ; it_tip != lstTip.end(); it_tip++, nRow++)
	{
		...
		Item.col = 3;
		Item.strText.Format(_T("%s"), ExtractKeyword(it_tip->title).c_str());
		m_Grid.SetItem(&Item);
	}
...
void CIndiCninfoAnnouncementRoadView::OnGridDblClick(NMHDR *pNotifyStruct, LRESULT* /*pResult*/)
{
