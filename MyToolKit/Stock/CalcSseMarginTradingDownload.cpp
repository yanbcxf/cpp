...
CCalcSseMarginTradingDownload::CCalcSseMarginTradingDownload(HWND hwnd, int logType)
:CCalculateTask(hwnd, logType)
{
}

CCalcSseMarginTradingDownload::~CCalcSseMarginTradingDownload(void)
{
}



void CCalcSseMarginTradingDownload::NeedDownload1(void* firstArg)
{
	CCalcSseMarginTradingDownload * sink = (CCalcSseMarginTradingDownload *)firstArg;
	sink->NeedDownload();
}



void CCalcSseMarginTradingDownload::NeedDownload()
{
	/*汇总需要下载 上海交易所的融资融券数据 的日期*/
	stringstream ss;
	...
	try{
		...
		ss << " from SseMarginTrading group by TradeDate order by TradeDate ";
