...
public:
	...
	const_pre_order_descendant_iterator() : pTop_node(0) {}
	// destructor, copy constructor, and assignment operator will be compiler generated correctly
protected:
	explicit const_pre_order_descendant_iterator(typename tree_category_type::const_iterator& it_, const tree_category_type* pTop_node_) : it(it_), pTop_node(pTop_node_) {}

public:
	// overloaded operators
	bool operator != ( const const_pre_order_descendant_iterator& rhs ) const { return it != rhs.it; }
	bool operator == ( const const_pre_order_descendant_iterator& rhs ) const { return it == rhs.it; }
	...
	// public interface
	const stored_type& operator*() const { return  it.operator *(); }
	const stored_type* operator->() const { return it.operator ->(); }
	const tree_type* node() const { return it.node(); }
...
public:
	...
	// destructor, copy constructor, and assignment operator will be compiler generated correctly
protected:
	explicit pre_order_descendant_iterator(typename tree_category_type::iterator& it_, const tree_category_type* const pTop_node_) : const_pre_order_descendant_iterator<stored_type, tree_type, container_type, tree_category_type>(it_, pTop_node_) {}
...
public:
	...
	// public interface
	stored_type& operator*() { return  const_cast<stored_type&>(it.operator *()); }
	stored_type* operator->() const { return const_cast<stored_type*>(it.operator ->()); }
	tree_type* node() const { return const_cast<tree_type*>(it.node()); }
	friend class associative_tree<stored_type, tree_type, container_type>;
	friend class sequential_tree<stored_type>;
};
...
protected:
	...
	explicit const_post_order_descendant_iterator(typename tree_category_type::const_iterator& it_, const tree_category_type* pTop_node_) : it(it_), pTop_node(pTop_node_) {}

public:
	// overloaded operators
	bool operator != ( const const_post_order_descendant_iterator& rhs ) const { return it != rhs.it; }
	bool operator == ( const const_post_order_descendant_iterator& rhs ) const { return it == rhs.it; }
...
public:
	...
	// destructor, copy constructor, and assignment operator will be compiler generated correctly
protected:
	explicit post_order_descendant_iterator(const tree_category_type* pTop_node_) : const_post_order_descendant_iterator<stored_type, tree_type, container_type, tree_category_type>(pTop_node_) { }
	explicit post_order_descendant_iterator(typename tree_category_type::iterator& it_, const tree_category_type* pTop_node_) : const_post_order_descendant_iterator<stored_type, tree_type, container_type, tree_category_type>(it_, pTop_node_) {}
...
public:
	// constructors/destructor
	const_level_order_descendant_iterator() : pTop_node(0) {}
	// destructor, copy constructor, and assignment operator will be compiler generated correctly
protected:
	explicit const_level_order_descendant_iterator(typename tree_category_type::const_iterator& it_, const tree_category_type* pTop_node_) : it(it_), pTop_node(pTop_node_) {}

public:
	// overloaded operators
	bool operator != (const const_level_order_descendant_iterator& rhs) const { return it != rhs.it; }
	bool operator == (const const_level_order_descendant_iterator& rhs) const { return it == rhs.it; }
...
public:
	...
	// destructor, copy constructor, and assignment operator will be compiler generated correctly
protected:
	explicit level_order_descendant_iterator(typename tree_category_type::iterator& it_, const tree_category_type* pTop_node_) : const_level_order_descendant_iterator<stored_type, tree_type, container_type, tree_category_type>(it_, pTop_node_) {}
