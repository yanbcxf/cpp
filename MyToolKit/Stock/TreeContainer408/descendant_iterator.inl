		...
		if ( rit != pTop_node->children.rend() ) { // insure there's children
			if ( !(*rit)->empty() ) { // last node have children?
				do {  // find the last child of this node
					...
					it = iterator_type(rit.base(), (it != pTop_node->end() ? node() : pTop_node)); // ..to convert to fwd iter correctly
		...
		if ( it != node()->parent()->begin() ) { // is this first sibling?
			...
			if (!it.node()->empty()) { // children present?
				do { // yes.  get deepest last child
					...
					it = iterator_type(node()->children.end(), node());
...
template<typename stored_type, typename tree_type, typename container_type, typename tree_category_type>
tcl::const_post_order_descendant_iterator<stored_type, tree_type, container_type, tree_category_type>::const_post_order_descendant_iterator(const tree_category_type* pTop_node_)
{
...
{
	const typename tree_category_type::const_iterator it_end = node()->parent()->end(); // end sibling
...
tcl::const_post_order_descendant_iterator<stored_type, tree_type, container_type, tree_category_type>& tcl::const_post_order_descendant_iterator<stored_type, tree_type, container_type, tree_category_type>::operator --()
{
	...
	if ( it == pTop_node->end() ) { // at end?
		...
		if ( !node()->empty() ) { // children present?
			...
			it = iterator_type(rit.base(), node());
...
{
	const typename tree_category_type::const_iterator it_end = node()->parent()->end(); 
