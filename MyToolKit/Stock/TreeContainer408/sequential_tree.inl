...
tcl::sequential_tree<stored_type>::insert( const stored_type& value)
{
	...
	allocate_tree_type(pNew_node, tree_type(value));
...
{
	// insert current node
	iterator base_it = insert(*tree_obj.get());
...
template<typename stored_type>
void 
tcl::sequential_tree<stored_type>::insert(const_iterator pos, const size_type num, const stored_type& value)
{
...
{
	// insert current node
	iterator base_it = insert(pos, *tree_obj.get());
...
// operator [](size_type)
template<typename stored_type>
tcl::sequential_tree<stored_type>& tcl::sequential_tree<stored_type>::operator [](size_type index) 
{ 
...
template<typename stored_type>
const tcl::sequential_tree<stored_type>& tcl::sequential_tree<stored_type>::operator [](size_type index) const 
{ 
	if (index >= basic_tree_type::size())
		throw std::out_of_range("sequential_tree index out of range");
...
// operator ==
template<typename stored_type>
bool tcl::sequential_tree<stored_type>::operator == (const tree_type& rhs) const
{
	// check this node
	if (!(*basic_tree_type::get() == *rhs.get()))
		return false;
	...
	for ( ; it != it_end && rhs_it != rhs_end; ++it, ++rhs_it ) {
		if (!(*it.node() == *rhs_it.node())) {
			return false;
		}
	}
...
bool tcl::sequential_tree<stored_type>::operator < (const tree_type& rhs) const
{
	...
	if (basic_tree_type::size() != rhs.size()) {
		return basic_tree_type::size() < rhs.size();
	}
