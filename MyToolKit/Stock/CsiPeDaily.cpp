...
string CCsiPeDaily::SqlScript()
{
	stringstream ss;
	ss << "create table if not exists CsiPeDaily (Code int , TradeDate date, is_stock int, " 
		<< " name varchar(128), company_number int,  parent_code int, "
...
bool CCsiPeDaily::Unzip(string zipfilename, string & xlsfilename)
{
		...
		default:                                        //文件
			{
				//创建文件
				CString strDiskFile;
				strDiskFile.Format("%s/%s/%s",g_strCurrentDir.c_str(), "CsiPeDaily", szZipFName);
...
int IndustryCode(string strCode)
{
	if(strCode.length()==2)
		strCode += "000000";
...
bool CCsiPeDaily::ParseXls(string  xlsfilename)
{
	...
	BasicExcelWorksheet* sheet1 = e.GetWorksheet(L"中证行业静态市盈率");
	if (sheet1)
	{
				...
				case BasicExcelCell::WSTRING:
					...
					if(r>0)
					{
						if(c==0)	
						{
							...
							strParent = "1" + strParent.substr(1);
							m_vec_code.insert(m_vec_code.end(), IndustryCode(strParent));
							strParent = strParent.substr(0, strParent.length()-2);
							m_vec_parent_code.insert(m_vec_parent_code.end(), IndustryCode(strParent));
	...
	BasicExcelWorksheet* sheet2 = e.GetWorksheet(L"中证行业滚动市盈率");
	if (sheet2)
	{
	...
	BasicExcelWorksheet* sheet3 = e.GetWorksheet(L"中证行业市净率");
	if (sheet3)
	{
	...
	BasicExcelWorksheet* sheet4 = e.GetWorksheet(L"中证行业股息率");
	if(sheet4==NULL)     sheet4 = e.GetWorksheet("中证行业股息率");
	if (sheet4)
	{
				...
				case BasicExcelCell::WSTRING:
					{
						...
						if(r>0)
						{
							if(c==0)	
							{
								m_vec_code.insert(m_vec_code.end(), atoi(str1.GetBuffer()));
							...
							else if(c==8)	
							{
								string strParent = str1.GetBuffer();
								strParent = "1" + strParent.substr(1);
								m_vec_parent_code.insert(m_vec_parent_code.end(), atoi(strParent.c_str()));
							}
							else if(c==10)	m_vec_static_pe.insert(m_vec_static_pe.end(), String2Double(str1.GetBuffer()));
							else if(c==11)	m_vec_dynamic_pe.insert(m_vec_dynamic_pe.end(),String2Double(str1.GetBuffer()));
							else if(c==12)	m_vec_pb.insert(m_vec_pb.end(), String2Double(str1.GetBuffer()));
							else if(c==13)	m_vec_dividend_yield_ratio.insert(m_vec_dividend_yield_ratio.end(), String2Double(str1.GetBuffer()));
						}
					}
					break;
				default:
...
string  CCsiPeDaily::SaveAsCsv()
{
	...
	char tmpCode[256] = {0};
	sprintf(tmpCode, "http://115.29.204.48/syl/csi%s.zip", strEnd.c_str());
	...
	if(zipLen>0)
	{
		...
		CString target;
		target.Format("%s/%s/csi%s.zip",g_strCurrentDir.c_str(), "CsiPeDaily", strEnd.c_str());
...
void CCsiPeDaily::ImportToDatabase(void)
{
	if(m_nCode<=0)
		return;
	...
	try
	{
		ss << "CCsiPeDaily::ImportToDatabase 开始导入数据.....\r\n";
		//sendToOutput(ss.str().c_str(), m_hWnd, m_nLogType);
		...
		{
			ss.str("");
			ss << "insert into CsiPeDaily ("
				<< "Code, TradeDate, is_stock, name, company_number, " 
				<< "parent_code, static_pe, dynamic_pe, pb, dividend_yield_ratio )"
				...
				<< " from dual where not exists (SELECT 1 from CsiPeDaily " 
				<< " where is_stock=" << m_vec_is_stock[row] << " "
				<< " and Code=" << Int2String(m_vec_code[row]) << " " 
				<< " and TradeDate=\'" << Int2String(m_nCode) << "\'  ) ";
		...
		ss << " OK, CCsiPeDaily::ImportToDatabase 数据导入完毕. \r\n";
		sendToOutput(ss.str().c_str(), m_hWnd, m_nLogType);
		//return EXIT_SUCCESS;
	}
...
string CCsiPeDaily::LastTradeDateFromDatabase()
{
	stringstream ss;
	backend_factory const &backEnd = *soci::factory_mysql();
	...
	try
	{
		...
		ss << "CCsiPeDaily::LastTradeDateFromDatabase 开始计算.....\r\n";
		sendToOutput(ss.str().c_str(), m_hWnd, m_nLogType);
		//////////////////////////////////////////////////////////////////////////
		ss.str("");
		ss << " select DATE_FORMAT(MaxDateForCsiPeDaily(), '%Y%m%d') as TradeDate  "
			<< " from dual " ;
	...
	{
		ss.str("");
		ss << "CCsiPeDaily::LastTradeDateFromDatabase maxdate = NULL.";
		ss << e.what() << ' \r\n';
		sendToOutput(ss.str().c_str(), m_hWnd, m_nLogType);
	}
...
void CCsiPeDaily::NotifyDatabaseImportedToWindow()
{
	stringstream ss;
...
CStockPlateTree * CCsiPeDaily::GenerateTree()
{
...
void CCsiPeDaily::ExportFromDatabase(double startTime, double endTime)
{
	m_vec_code.clear();
	m_vec_name.clear();
	m_vec_is_stock.clear();
	...
	try
	{
		...
		//session sql(backEnd, m_ConnectString);
		session sql(g_MysqlPool);
		sql << "call add_ind('csipedaily', 'tradedate', 'idx_tradedate')";
		...
		while (st.fetch())
		{
			m_vec_code.insert(m_vec_code.end(), r.get<int>("Code"));
			...
			m_vec_parent_code.insert(m_vec_parent_code.end(), r.get<int>("parent_code"));
