...
void COwspClient::handleDestoryed()
{
	...
	CString msg;
	msg.Format("clientName（%s）,设备（%s）", name(), m_strDevSn.c_str());
	envir().AppendLog(msg);
}
...
unsigned COwspClient::sendLoginCommand() 
{
	...
	return sendRequest(new RequestRecord(/*++envir().m_nSeq*/ envir().GetSeq(), OwspCallback, TLV_T_LOGIN_REQUEST,
