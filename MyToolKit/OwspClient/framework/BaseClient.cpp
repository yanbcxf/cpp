...
unsigned BaseClient::responseBufferSize = 50 * 1024;  //1.5 * 1024 * 1024; // default value; you can reassign this in your application if you need to
...
BOOL BaseClient::ASend( const unsigned char * buf, int len )
{
	...
	while(left>0)
	{
		...
		if(pOverlapBuff)
		{
			...
			CString msg;
			msg.Format("BaseClient::ASend , len = %d, Used = %d ", len, pOverlapBuff->GetUsed());
			envir().AppendLog(msg);
...
BOOL BaseClient::ARead(CIOCPBuffer *pOverlapBuff)
{
	...
	if(m_Socket!=INVALID_SOCKET )
	{
		...
		if(pOverlapBuff==NULL) 
		{			
			envir().AppendLog("AllocateBuffer(IORead) == NULL.");
			return FALSE;
		}
...
void BaseClient::OnWrite(DWORD dwIoSize,CIOCPBuffer *pOverlapBuff)
{
	...
	{
		CString msg;
		msg.Format(" BaseClient::OnWrite , dwIoSize = %d, Used = %d", dwIoSize, pOverlapBuff->GetUsed());
		envir().AppendLog(msg);
		...
		if ( nRetVal == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING ) 
		{
			if(WSAGetLastError()!=WSAENOTSOCK)
			{
				...
				{	
					CString msg;
					msg.Format("Error in OnWrite..: %s",envir().ErrorCode2Text(WSAGetLastError()));
					envir().AppendLog(msg);
				}
			}
			//Disconnect();
		...
		{
			CString msg;
			msg.Format("发送被推迟 in OnWrite..: %s",envir().ErrorCode2Text(WSAGetLastError()));
			envir().AppendLog(msg);
		}
		else
		{
			//	异步发送数据成功
			CString msg;
			msg.Format("发送成功 in OnWrite..: 共发送 %d 字节", dwSendNumBytes);
			envir().AppendLog(msg);
		}
	}
...
void BaseClient::OnWriteCompleted( DWORD dwIoSize,CIOCPBuffer *pOverlapBuff)
{
	if (pOverlapBuff!=NULL)
	{
		...
		{
			CString msg;
			msg.Format("The whole message was not Sent.: %s",envir().ErrorCode2Text(WSAGetLastError()));
			envir().AppendLog(msg);
		}
		else
		{
		...
		CString msg;
		msg.Format(" BaseClient::OnWriteCompleted, 发送完成后，回收缓存，dwIoSize = %d , Used = %d", dwIoSize, \
	...
	if(m_Socket!=INVALID_SOCKET && pOverlapBuff)
	{
		...
		if ( nRetVal == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING) 
		{
			if(WSAGetLastError()!=WSAENOTSOCK)
			{
				...
				{
					CString msg;
					msg.Format("Disconnect in Onread Possible Socket Error: %s",envir().ErrorCode2Text(WSAGetLastError()));
					envir().AppendLog(msg);
				}
			}
			g_BufferMgr.ReleaseBuffer(pOverlapBuff);
...
void BaseClient::OnReadCompleted(DWORD dwIoSize,CIOCPBuffer *pOverlapBuff)
{
	...
	{
		CString msg;
		msg.Format("Disconnection in OnReadCompleted (dwIoSize == 0||pOverlapBuff==NULL) errcode: %s",envir().ErrorCode2Text(WSAGetLastError()));
		envir().AppendLog(msg);
		//Disconnect();
		Reclaim();
		g_BufferMgr.ReleaseBuffer(pOverlapBuff);
...
int BaseClient::connectToServer_Old(SOCKET	socketNum, int remotePortNum) 
{
	...
	if (nRet == SOCKET_ERROR)
	{
		...
		{
			CString msg;
			msg.Format("Connection Error: %s",envir().ErrorCode2Text(WSAGetLastError()));
			envir().AppendLog(msg);
			return -1;
		}
		else
...
int BaseClient::connectToServer(SOCKET	socketNum, int remotePortNum) 
{
	...
	if(pOverlapBuff==NULL) 
	{			
		envir().AppendLog("AllocateBuffer(IOConnect) == NULL.");
		return -1;
	}
	...
	{ 
		...
		{ 
			envir().AppendLog("ConnextEx error ");
			return -1 ; 
		} 
		else //  操作未决（正在进行中 … )
		{ 
...
int BaseClient::openConnection()
{
	do{
		...
		{
			CString msg;
			msg.Format("AssociateClientWithCompletionPort Failed: %s",envir().ErrorCode2Text(WSAGetLastError()));
			envir().AppendLog(msg);
			envir().AppendInfoLog(msg);
			break;
		}
...
int BaseClient::openConnection_Old()
{
	do{
		...
		{
			CString msg;
			msg.Format("setsockopt() error: %s",envir().ErrorCode2Text(WSAGetLastError()));
			envir().AppendLog(msg);
			break;
		}
