...
void CTParserBufferManager::ResetVisitBuffer()
{
	if(m_pu8CacheBuffer)
	{
...
{
	// LOG4CPLUS_DEBUG(gb_Logger, "PopParsedBuffer");
...
{
	if(m_pu32OutputBuffer)
		delete []m_pu32OutputBuffer;
}
		...
		if(pu8Point[0] == 0x00 && pu8Point[1] == 0x00 && pu8Point[2] == 0x01 && pu8Point[3] == SYSTEM_HEADER_START_CODE)
		{
			...
			if(pu8Point[0] == 0x00 && pu8Point[1] == 0x00 && pu8Point[2] == 0x01 && pu8Point[3] == MAP_STREAM_ID)
			{
				...
				while (elementary_stream_map_length >= 4)
				{
					...
					{
						char tmp[256] = { 0 };
						sprintf_s(tmp, 256, "MAP_STREAM_ID [%d] = %d",elementary_stream_id, stream_type);
					...
					{
						if(elementary_stream_map_length >= elementary_stream_info_length)
							elementary_stream_map_length -= elementary_stream_info_length;
						else
						{
							elementary_stream_map_length = 0;
							LOG4CPLUS_DEBUG(gb_Logger, "elementary_stream_map_length 长度不匹配");
						}
					}
				}
				m_PacketCounter++;
...
int CTActivePSParser::ParsePESPacket()
{
	...
	{	
		do 
		{
			READ_BYTE(u8Value, ParsePESPacketFail_1);
		} while (u8Value != 0x01);

		// yangbin 防止缓存中仅有单字节 且为 0x01 时，无法确定 pes 包头
		PopParsedBuffer();

		SEEK_BYTES(-3, ParsePESPacketFail_2);

		PopParsedBuffer();

		READ_BYTES(pu8Point, 3, ParsePESPacketFail_3);
		if (pu8Point[0] == 0x00 && pu8Point[1] == 0x00 && pu8Point[2] == 0x01)
		{
			READ_BYTE(u8Value, ParsePESPacketFail); // 224 : E0 | 192 : C0 （音频）
			if(u8Value == 0xBA)
			{
				m_PacketCounter++;
			}
			if(m_StreamIDMap[u8Value] != 0)
			{
				...
				{
					LOG4CPLUS_DEBUG(gb_Logger, "not correct 1");
					continue;
				}
				if((PES_packet_length - PES_header_data_length) <= 3)
				{
					LOG4CPLUS_DEBUG(gb_Logger, "not correct 2");
					continue;
				}
				...
				if(m_bMergeFlag && u8Value == m_u8MergeID && es_data_len < MERGE_BUFFER_SIZE)
				{
					...
					stPESPacket.es_data = m_pu32OutputBuffer;
				...
				if (m_cbDataHandleFuncs)
				{
					...
					{
						char tmp[256] = { 0 };
						sprintf_s(tmp, 256, "m_cbDataHandleFuncs，用时 %d ms", delay);
						LOG4CPLUS_DEBUG(gb_Logger, string(tmp) );
					}
	...
	ss.str("");
	ss << "ParsePESPacket Ok" ;
	//LOG4CPLUS_DEBUG(gb_Logger, ss.str());
	return 0;

ParsePESPacketFail_1:
	ss.str("");
	ss << "ParsePESPacket Fail_1";
	//LOG4CPLUS_DEBUG(gb_Logger, ss.str());
	return -1;

ParsePESPacketFail_2:
	ss.str("");
	ss << "ParsePESPacket Fail_2";
	//LOG4CPLUS_DEBUG(gb_Logger, ss.str());
	return -1;

ParsePESPacketFail_3:
	ss.str("");
	ss << "ParsePESPacket Fail_3";
	//LOG4CPLUS_DEBUG(gb_Logger, ss.str());
	return -1;
...
int CTActivePSParser::WriteInOutputBuffer(unsigned char *pu8Data, unsigned int u32DataLength)
{
	...
	if(!m_pu32OutputBuffer)
	{
		m_pu32OutputBuffer = new unsigned char[m_u32OutputBufferSize = ((u32DataLength + MERGE_BUFFSIZE_INC_UNIT) / MERGE_BUFFSIZE_INC_UNIT * MERGE_BUFFSIZE_INC_UNIT)];
		memcpy(m_pu32OutputBuffer, pu8Data, u32DataLength);
	...
	else
	{
		if(m_u32OutputBufferSize < (m_u32OutputBufferDataLength + u32DataLength))
		{
			...
			//ASSERT(pu8Tmp);
			memcpy(pu8Tmp, m_pu32OutputBuffer, m_u32OutputBufferDataLength);

			delete []m_pu32OutputBuffer;
			m_pu32OutputBuffer = pu8Tmp;
		}
		
		memcpy(m_pu32OutputBuffer + m_u32OutputBufferDataLength, pu8Data, u32DataLength);
		m_u32OutputBufferDataLength += u32DataLength;
	}
...
//	该解析器的 唯一入口
int CTActivePSParser::ParsePSData(unsigned char *pu8PSData, unsigned int u32PSDataLength, uint32_t rtptime)
{
	stringstream ss;
	...
	/*ss.str("");
	ss << "ParsePSData 开始, m_u32CacheDataLength = " <<  m_u32CacheDataLength;
	ss << ", m_u32DataLength = " << m_u32DataLength;
	ss << ", m_enReadSegment = " << (m_enReadSegment==1? "current" : "buffer");
	LOG4CPLUS_DEBUG(gb_Logger, ss.str());*/
