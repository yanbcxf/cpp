...
bool CompoundFile::Create(const char* filename)
{
	...
	// yangbin 简化版对中文字符的转换不好, 该函数最好不要调用
...
size_t SmallString::Read(const char* data)
{
	...
	LittleEndian::Read(data, stringSize1, 0, 1);
	stringSize = (unsigned char)stringSize1;
	...
	while (code != CODE::YEOF)
	{
		switch (code)
		{
			...
			case CODE::DIMENSIONS:
				...
				/* yangbin 2016-07-25， 多增加一列，避免从“深交所”下载融资融券的 xls 是出错 */
				dimensions_.lastUsedColIndexPlusOne_ ++;
				break;
