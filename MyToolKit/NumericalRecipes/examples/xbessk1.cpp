...
int main(void)
{
        ...
        getline(fp,txt);
        while (txt.find("Modified Bessel Function K1")) {
          getline(fp,txt);
          if (fp.eof()) NR::nrerror("Data not found in fncval.dat");
        }
        fp >> nval;
        getline(fp,txt);
        cout << endl << "Modified Bessel Function K1" << endl;
        cout << setw(5) << "x" << setw(14) << "actual";
        cout << setw(18) << "bessk1(x)" << endl << endl;
        for (i=0;i < nval;i++) {
          fp >> x >> val;
          cout << fixed << setprecision(2);
          cout << setw(6) << x;
          cout << scientific << setprecision(7);
          cout << setw(17) << val << setw(17) << NR::bessk1(x) << endl;
        }
        fp.close();
        return 0;
}
