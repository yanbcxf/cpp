...
void Node::parseAttributes() 
{
	...
	while (!isspace(*ptr)&& *ptr != '>' ) ++ptr;   // yangbin 2015-12-27  增加了 *ptr != '>'
...
//				fprintf(stderr, "Trying to find quote: %c\n", quote);
				...
				const char *begin = ptr + 1;			//	yangbin 2016-7-22   在 isspace 函数中增加强制转换 （unsigned char）  
				while (isspace((unsigned char)*begin) && begin < end) ++begin;
				const char *trimmed_end = end - 1;		//	yangbin 2016-7-22   在 isspace 函数中增加强制转换 （unsigned char）
				while (isspace((unsigned char)*trimmed_end) && trimmed_end >= begin) --trimmed_end;
