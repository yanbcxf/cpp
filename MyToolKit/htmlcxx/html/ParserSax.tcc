...
//						DEBUGP("Parsing comment\n");
			...
			if (*c == '<')
			{
				...
				if (d != end)
				{
					if (*d>0 && *d <256 && isalpha(*d))     // yangbin 2016-5-22
					{
						// beginning of tag
						if (begin != c)
...
htmlcxx::HTML::ParserSax::skipHtmlComment(_Iterator c, _Iterator end)
{
	while ( c != end ) {
		...
		{
			_Iterator d(c);
			while (++c != end && *c>0 && *c <256 &&  isspace(*c));   // yangbin 2016-10-3 新增判断 *c>0 && *c <256
			if (c == end || *c++ == '>') break;
			c = d;
		}
	}
...
_Iterator htmlcxx::HTML::ParserSax::skipHtmlTag(_Iterator c, _Iterator end)
{
	while (c != end && *c != '>')
	{
		...
		{ // found an attribute
			++c;
			while (c != end && isspace((unsigned char)*c)) ++c;			// yangbin 2016-7-22 增加了强制转换 （unsigned char）
