...
void CDMGraphCtrl::DrawGraphOffScreen(HDC hDC, LPCRECT prcBounds, BOOL bOptimized)
{
   ...
   BOOL bMetafile = GetDeviceCaps(hDC, TECHNOLOGY) == DT_METAFILE;
   if(bMetafile)
   {
	   // Save the metafile DC state, since DrawGraph might change it.
	   int iSavedDC;
	   iSavedDC = SaveDC(hDC);

	   // Draw our control on the metafile DC.
	   DrawGraph(hDC, &rcBitmapBounds, bOptimized) ;

	   // Restore the DC, since DrawGraph might have changed mapping modes.
   ...
   else
   {
	   ...
	   if(dcMem == NULL)
	   {
		   ATLTRACE(_T("Cannot CreateCompatibleDC from %#x err %d\n"), hDC, GetLastError());
		   return;
	   }
	   ...
	   if(bitOff == NULL)
	   {
		   ATLTRACE(_T("Cannot CreateCompatibleBitmap from %#x err %d\n"), hDC, GetLastError());
		   goto $end;
	   }

	   // Select the bitmap into the memory DC.
	   HBITMAP hOldBitmap;
	   hOldBitmap = (HBITMAP)SelectObject(dcMem, bitOff) ;

	   // Save the memory DC state, since DrawGraph might change it.
	   int iSavedDC;
	   iSavedDC = SaveDC(dcMem);
...
void CDMGraphCtrl::PlotElementPoints(HDC hDC, CGraphElement* pElement, BOOL bOptimized)
{
	...
	//Start plot all available data.
	int i;
	for(i=0; i<pElement->m_PointList.GetSize(); i++)
	{
		pt = pElement->m_PointList[i];
...
LRESULT CDMGraphCtrl::OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	::SetClassLongPtr(m_hWnd, -12 /*GCL_HCURSOR*/, (LONG_PTR)LoadCursor(NULL, IDC_CROSS));
	return 0;
}
...
void CDMGraphCtrl::CursorPosition(const LPPOINT point) 
{
	...
	{	
		...
		{
			if (pCursor->m_nMode == Snap && m_pElementList->GetSize() > 0)
			{		
				if(FindPoint(rx,ry, &pt))
				{
					rx = pt.x;
					ry = pt.y;
				}
