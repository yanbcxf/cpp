...
void CBitmapSlider::OnPaint() 
{
	...
	if( m_bChannel )
	{
		...
		{
			// Lower part
			DrawBitmap( &dc, 0, Pos2Pixel(m_nPos),m_nWidth, m_nHeight - Pos2Pixel(m_nPos),&dcMem, 0, Pos2Pixel(m_nPos),&m_bmChannelActiveMask, m_bTransparentChannel );
			dcMem.SelectObject( &m_bmChannelActive );
			// Upper part
			DrawBitmap( &dc, 0, 0, m_nWidth, Pos2Pixel(m_nPos),&dcMem, 0, 0, &m_bmChannelMask, m_bTransparentChannel );			
		}
		else 
		{
			// Horizontal slider
			// Right side
			DrawBitmap( &dc, Pos2Pixel(m_nPos), 0,m_nWidth - Pos2Pixel(m_nPos), m_nHeight,&dcMem, Pos2Pixel(m_nPos), 0,&m_bmChannelActiveMask, m_bTransparentChannel );
			dcMem.SelectObject( &m_bmChannelActive );
			// Left side
			DrawBitmap( &dc, 0, 0, Pos2Pixel(m_nPos), m_nHeight,&dcMem, 0, 0, &m_bmChannelMask, m_bTransparentChannel );
		}
	...
	if( m_nThumbBgX != -1 ) 
	{
		RestoreBackground(&dc, m_nThumbBgX, m_nThumbBgY,m_nThumbWidth, m_nThumbHeight, &m_bmThumbBg );
	...
	if( m_bThumb) 
	{
		...
		if( m_bVertical ) 
		{
			...
			DrawBitmap(&dc, m_nMarginLeft, Pos2Pixel(m_nPos) - m_nThumbHeight/2,m_nThumbWidth, m_nThumbHeight,&dcMem, 0, 0, &m_bmThumbMask, m_bTransparentThumb );
		} 
		else
		{
			// Horizontal slider
			...
			DrawBitmap(&dc, Pos2Pixel(m_nPos) - m_nThumbWidth/2, m_nMarginTop,m_nThumbWidth, m_nThumbHeight,&dcMem, 0, 0, &m_bmThumbMask, m_bTransparentThumb );
		} // if horizontal
...
//
BOOL CBitmapSlider::SetBitmapChannel(UINT nChannelID, UINT nActiveID , BOOL bTransparent,COLORREF clrpTransColor, int iTransPixelX, int iTransPixelY )
{
	...
	if( bTransparent ) 
	{
		PrepareMask( &m_bmChannel, &m_bmChannelMask,clrpTransColor, iTransPixelX, iTransPixelY );
	}
	...
	if( nActiveID ) 
	{
		...
		if( bTransparent ) 
		{
			PrepareMask( &m_bmChannelActive, &m_bmChannelActiveMask,clrpTransColor, iTransPixelX, iTransPixelY );
		}
	...
	COLORREF clrpTransColor, int iTransPixelX, int iTransPixelY )
{
	...
	if( bTransparent ) 
	{
		PrepareMask( &m_bmThumb, &m_bmThumbMask,clrpTransColor, iTransPixelX, iTransPixelY );
	}
	...
	if( nActiveID ) 
	{
		...
		if( bTransparent ) 
		{
			PrepareMask( &m_bmThumbActive, &m_bmThumbActiveMask,clrpTransColor, iTransPixelX, iTransPixelY );
		}
	...
	if( m_bThumbActive ) 
	{
		...
		ASSERT(m_nThumbWidth == bitmap.bmWidth &&m_nThumbHeight == bitmap.bmHeight );
	}
...
void CBitmapSlider::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	DebugTrace("OnLButtonDown ---- 1---- m_nPos=%d ",m_nPos);
	if( m_bVertical ) 
	{
	...
	Invalidate();
    DebugTrace("OnLButtonDown  ---- 2---- m_nPos=%d ",m_nPos);
...
void CBitmapSlider::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if( m_bLButtonDown && m_bEnable )
	{
		...
		if( Pos2Pixel(m_nPos) != nPixel ) 
		{
			...
			::PostMessage(GetParent()->GetSafeHwnd(), WM_BITMAPSLIDER_MOVING,GetDlgCtrlID(), m_nPos );
		}
	}

	if (nFlags & MK_LBUTTON && !m_bMouseOnButton) return;
	...
	if(m_bMouseOnButton || m_bLButtonDown)
	{
		CString strToolTip;
		if(m_bShowToolTipPos)
		{
			strToolTip.Format(_T("%s:%d"),m_strToolTip,m_nPos);
			switch(m_nPos)
			{
			case 0:
				strToolTip.Format(_T("%s:1/16倍速"),m_strToolTip);
				break;
			case 1:
				strToolTip.Format(_T("%s:1/8倍速"),m_strToolTip);
				break;
			case 2:
				strToolTip.Format(_T("%s:1/4倍速"),m_strToolTip);
				break;
			case 3:
				strToolTip.Format(_T("%s:1/2倍速"),m_strToolTip);
				break;
			case 4:
				strToolTip.Format(_T("%s:1倍速"),m_strToolTip);
				break;
			case 5:
				strToolTip.Format(_T("%s:2倍速"),m_strToolTip);
				break;
			case 6:
				strToolTip.Format(_T("%s:4倍速"),m_strToolTip);
				break;
			case 7:
				strToolTip.Format(_T("%s:8倍速"),m_strToolTip);
				break;
			case 8:
				strToolTip.Format(_T("%s:16倍速"),m_strToolTip);
				break;
			}
		}
		else
...
void CBitmapSlider::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	::PostMessage(GetParent()->GetSafeHwnd(),	WM_BITMAPSLIDER_MOVED,GetDlgCtrlID(), m_nPos );
	...
	CDC *pTmpDC, int xSource, int ySource, CBitmap *bmMask, BOOL bTransparent )
{
	if( bTransparent ) 
	{
		DrawTransparentBitmap(pDC, xStart, yStart,wWidth, wHeight,pTmpDC, xSource, ySource, bmMask );
	}
	else 
	{
...
//
void CBitmapSlider::PrepareMask(CBitmap *pBmpSource, CBitmap *pBmpMask,COLORREF clrpTransColor, int iTransPixelX, int iTransPixelY)
{
	CDC hdcSrc, hdcDst;
...
void CBitmapSlider::SetTooltipText(CString strText)
{
	if(strText.IsEmpty())
	{
		return;
