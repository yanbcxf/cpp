...
typedef struct sipd_sender_sdp_info
{
	...
	char media_sender_or_receiver_id[MAX_SIPD_DEVICE_ID_LEN+1];
	/*媒体流发送者IP地址类型*/
	enum SIPD_IPType media_send_or_receive_ip_type;
	/*媒体流发送者IP地址*/
	char media_send_or_receive_ip[MAX_SIPD_URI];
    /*媒体流发送者RTP端口号*/
	char media_send_or_receive_port[MAX_SIPD_NORMAL_LEN];
	char ssrc[MAX_SIPD_NORMAL_LEN];
...
typedef int (* Sipd_GetSIPServerCfg_func)(SIPD_GATEWAY_CFG* pCfg);
typedef int (* Sipd_settime_func)(const sipd_id_info * pDeviceID,  const SIPD_TIME* pTime);
typedef int (* Sipd_PTZ_Move_func)(const sipd_ptz_control* pControl);
typedef int (* Sipd_PTZ_Tour_func)(const sipd_tour_control * pTourControl);
typedef int (* Sipd_PTZ_Preset_func)(const sipd_preset_control * pPresetControl);
typedef int (* Sipd_PTZ_AutoScan_func)(const sipd_autoscan_control * pAutoScanControl);
typedef int (* Sipd_Reboot_func)(const sipd_id_info * pIdInfo);
typedef int (* Sipd_record_control_func) (const sipd_record_control * pRecordControl);
typedef int (* Sipd_alarm_control_func) (const sipd_alarm_control * pAlarmControl);


typedef int (* Sipd_cataloginfo_request_func)(const sipd_catalog_req * pCatalogReq);
typedef int (* Sipd_deviceinfo_request_func)(const sipd_deviceinfo_req * pDeviceinfoReq);
typedef int (* Sipd_stateinfo_request_func)(const sipd_devicestate_req * pStateInfoReq);


typedef int (* Sipd_search_recordfile_req_func) (sipd_recordinfo_req * pRecordFileReq);
typedef int (* Invite_play_playback_download_func)(const sipd_media_session* pMediaInfo);

typedef int (* Sipd_media_control_notify_CMS_func)(const int did, const int tid, const enum SIPD_TO_CMS_CMD_TYPE cmd_type, const sipd_playback_mans_rtsp_ind* playback_rtsp_control);
typedef int (* Sipd_register_req_func) (const sipd_register_req * pRegisterReq);

typedef int (* sipd_message_query_or_response_req_func) (const sipd_message_query_or_response * pMessageResponseReq);
typedef int (* sipd_on_idle_func)();
