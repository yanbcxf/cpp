...
void CBaseGridCtlView::RefreshStatusBar()
{
	CCostEngineerApp * pDoc = (CCostEngineerApp *)GetDocument();
	/*vector<LotteryInfo>	vecInfo = ((CLotteryApp *)AfxGetApp())->m_vec_lottery_info;

	CMainFrame * pFrm = (CMainFrame *)AfxGetMainWnd();
	stringstream ss;
	ss << pDoc->m_lottery_info.name;
	if (m_start_number > 0)
		ss << ", 开始期数 " << m_start_number;
	if (m_end_number > 0)
		ss << ", 结束期数 " << m_end_number;
	...
	if (pDoc->m_strDataSource.IsEmpty())
		ss << ", 数据源 No";
	else
		ss << ", 数据源 " << pDoc->m_strDataSource.GetBuffer();

	if (pDoc->m_strFilterSource.IsEmpty())
		ss << ", 过滤源 No";
	else
		ss << ", 过滤源 " << pDoc->m_strFilterSource.GetBuffer();

	pFrm->SetStatusBarText(ss.str().c_str());*/
}



void CBaseGridCtlView::Startup()
{
	CCostEngineerApp * pDoc = (CCostEngineerApp *)GetDocument();
	/*CCalcLotteryNumberList * cse = new CCalcLotteryNumberList(m_hWnd, WM_USER_LOG_1);
	cse->m_lottery_info = pDoc->m_lottery_info;
	...
	int nFred = pDoc->m_fred_select;
	if (nFred >= 0 && nFred < 4 && nFred <  pDoc->m_fred_alpha.size())
	{
		cse->m_alpha = pDoc->m_fred_alpha[nFred];
		cse->m_beta = pDoc->m_fred_beta[nFred];
		cse->m_upsilon = pDoc->m_fred_upsilon[nFred];
		cse->m_theta = pDoc->m_fred_theta[nFred];
		cse->m_eta = pDoc->m_fred_eta[nFred];
	}
