...
// yangbin : 获取x 轴上某段曲线的 Y 轴最大最小值
template <class T>
bool CChartSerieBase<T>::GetYMinMaxForXMinMax(double xMin, 
										  double xMax,
										  double& yMin, 
										  double& yMax) 
{
	...
	if(m_vPoints.GetOrdering()==poXOrdering)
	{
		bResult = m_vPoints.GetVisiblePoints(xMin, xMax, uFirst, uLast);
	}
...
template<class T>
int CChartSerieBase<T>::SearchPointWithCustomLabel(unsigned uPointIndex)
{
	//	寻找该点附近有“气球提示”的点
	if(uPointIndex<GetPointsCount())
	{
		for(int i = 0; i<10; i++)
		{
			if(uPointIndex + i <= GetPointsCount() -1)
			{
				TLabelMap::iterator iter = m_mapLabels.find(uPointIndex + i);
				if (iter != m_mapLabels.end())
					return uPointIndex + i;
			}
