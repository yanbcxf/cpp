...
static SDL_Texture * sdlTextureBorder;
static SDL_Surface * sdlSurfaceBorder;
                                  ...
                                  int x, int y, int w, int h, int color, int update)
{
	...
	if (update && w > 0 && h > 0)
	{
		...
		SDL_RenderCopy(sdlRenderer, sdlTextureBorder, NULL, &rect);
	}
}
...
static void video_image_display(VideoState *is)
{
    ...
    if (vp->bmp) {
		...
		SDL_RenderCopy(sdlRenderer, vp->bmp, NULL, &rect);
        ...
        if (rect.x != is->last_display_rect.x || rect.y != is->last_display_rect.y || rect.w != is->last_display_rect.w || rect.h != is->last_display_rect.h || is->force_refresh) {
			int bgcolor = SDL_MapRGB(sdlSurfaceBorder->format, 0x00, 0x00, 0x00);
...
static int video_open(VideoState *is, int force_set_video_mode, Frame *vp)
{
   ...
   /* screen = SDL_SetVideoMode(w, h, 0, flags);
	...
	sdlWindow = SDL_CreateWindow("", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, w, h, flags);
	if (!sdlWindow) {
	...
	SDL_SetWindowTitle(sdlWindow, window_title);

	//	创建边框的填充面
	sdlTextureBorder = SDL_CreateTexture(sdlRenderer, SDL_PIXELFORMAT_RGB24, SDL_TEXTUREACCESS_STREAMING,
		640, 480);

	sdlSurfaceBorder = SDL_CreateRGBSurface(0, 640, 480, 24,
		0x00FF0000,
		0x0000FF00,
		0x000000FF,
		0xFF000000);

	if (!sdlTextureBorder || !sdlSurfaceBorder) {
		av_log(NULL, AV_LOG_FATAL, "SDL: could not set sdlTextureBorder or sdlSurfaceBorder - exiting\n");
		do_exit(is);
	}

	SDL_TimerID timer = SDL_AddTimer(100, refresh_callback, is);
...
static void video_display(VideoState *is)
{
	...
	SDL_RenderClear(sdlRenderer);	// SDL 2.0
	if (is->audio_st && is->show_mode != SHOW_MODE_VIDEO)
        video_audio_display(is);
    else if (is->video_st)
        video_image_display(is);	
	SDL_RenderPresent(sdlRenderer);	// SDL 2.0
}
...
static void alloc_picture(VideoState *is)
{
    ...
    /*vp->bmp = SDL_CreateYUVOverlay(vp->width, vp->height,
                                   SDL_YV12_OVERLAY,
                                   screen);*/
	vp->bmp = SDL_CreateTexture(sdlRenderer, /*SDL_PIXELFORMAT_YV12*/SDL_PIXELFORMAT_IYUV,
...
static void refresh_loop_wait_event(VideoState *is, SDL_Event *event) {
   ...
   //     remaining_time = REFRESH_RATE;	/*10ms*/
...
static void seek_chapter(VideoState *is, int incr)
{
    ...
    for (i = 0; i < is->ic->nb_chapters; i++) {
		...
		AVRational avr;
		avr.num =1; avr.den = AV_TIME_BASE;
        if (av_compare_ts(pos, avr/*AV_TIME_BASE_Q*/, ch->start, ch->time_base) < 0) {
            i--;
            break;
        }
    }
...
//#include "cmdutils_common_opts.h"
	...
	{ "ast", OPT_INT | HAS_ARG | OPT_EXPERT, { &wanted_stream_spec[AVMEDIA_TYPE_AUDIO] }, "select desired audio stream", "stream_number" },
	{ "vst", OPT_INT | HAS_ARG | OPT_EXPERT, { &wanted_stream_spec[AVMEDIA_TYPE_VIDEO] }, "select desired video stream", "stream_number" },
	{ "sst", OPT_INT | HAS_ARG | OPT_EXPERT, { &wanted_stream_spec[AVMEDIA_TYPE_SUBTITLE] }, "select desired subtitle stream", "stream_number" },
