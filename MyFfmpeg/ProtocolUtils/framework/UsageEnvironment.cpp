...
UINT __stdcall UsageEnvironment::IOWorkerThreadProc(LPVOID pParam)
{
	...
	if ( pThis )
	{
		...
		{
			pThis->AppendLog(_T("===================== IOWorker Start!"));
		}
		else
		{
			pThis->AppendLog(_T("===================== IOWorker CreateCompletionPort 失败!"));
			return 0xdead;
		}
		...
		while ( !bError )
		{
			...
			CString msg;
			msg.Format(_T("GetQueuedCompletionStatus 开始执行 ,允许 Timeout = %d"), waitTime);
			//pThis->AppendLog(msg);
			...
			if (!bIORet)  
			{
				...
				{	
					...
					bError = true;
					pThis->AppendLog(pThis->ErrorCode2Text(dwIOError)); 
					pThis->AppendLog(_T("IOWORKER KILLED BECAUSE OF ERROR IN GetQueuedCompletionStatus"));
			...
			{
				pThis->AppendLog(_T("lpClientContext==NULL \r\n"));
				bError=true;
			}
	...
	pThis->AppendLog(_T("================================= IOWorker Died!"));
	return 0xdead;
}
...
{
	HANDLE h = CreateIoCompletionPort((HANDLE) pBaseClient->m_Socket, m_hCompletionPort, (DWORD)pBaseClient, 0);
...
BOOL UsageEnvironment::PostQueuedCompletionStatus(BaseClient * pBaseClient, CIOCPBuffer *pOverlapBuff)
{
	if(pOverlapBuff!=NULL)
	{
		...
		{            
			CString msg;
			msg.Format(_T("Error in UsageEnvironment::PostQueuedCompletionStatus: %s"), ErrorCode2Text(WSAGetLastError()));
			AppendLog(msg);
			return FALSE;
		}
		return TRUE;
...
void UsageEnvironment::ProcessIOMessage(CIOCPBuffer *pOverlapBuff, PVOID pContext, DWORD dwSize)
{
	...
	case IOInitialize :
		AppendLog(_T("UsageEnvironment::ProcessIOMessage ---> IOInitialize"));
		((BaseClient *)pContext)->OnInitialize(pOverlapBuff);
		break;

	case IORead :
		AppendLog(_T("UsageEnvironment::ProcessIOMessage ---> IORead"));
		((BaseClient *)pContext)->OnRead(pOverlapBuff);
		break;	


	case IOReadCompleted :
		AppendLog(_T("UsageEnvironment::ProcessIOMessage ---> IOReadCompleted"));
		((BaseClient *)pContext)->OnReadCompleted(dwSize,pOverlapBuff);
		break;


	case IOWrite :
		AppendLog(_T("UsageEnvironment::ProcessIOMessage ---> IOWrite"));
		((BaseClient *)pContext)->OnWrite(dwSize,pOverlapBuff);
		break;

	case IOWriteCompleted :
		AppendLog(_T("UsageEnvironment::ProcessIOMessage ---> IOWriteCompleted"));
	...
	case IOCommand : //  执行 SDK 接口投递的命令.. 
		AppendLog(_T("UsageEnvironment::ProcessIOMessage ---> IOCommand"));
