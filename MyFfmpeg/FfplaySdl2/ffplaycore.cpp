...
static int videostate_list_get_by_sessionId(VideoStateList *q, VideoState **pkt, uint32_t sessionId)
{
	VideoState *pkt1, *pkt2;
	int ret = 0;
...
static void video_image_display(VideoState *is)
{
    ...
    if (vp->bmp) {
		...
		//calculate_display_rect(&rect, is->xleft, is->ytop, is->original_width, is->original_height, vp->width, vp->height, vp->sar);

        //SDL_DisplayYUVOverlay(vp->bmp, &rect);
		SDL_Rect rect1;
		rect1.x = rect1.y = 0;
		rect1.w = is->width-10;
		rect1.h = is->height-10;
...
static void video_display(VideoState *is)
{
	...
	SDL_RenderClear(is->sdlRenderer);	// SDL 2.0
...
static VideoState *stream_open(CustomEventData * data  /*, const char *filename, AVInputFormat *iformat*/)
{
	...
	is->sdlRenderer = data->renderer;
	/*is->sdlTextureBorder = data->sdlTextureBorder;
	is->sdlSurfaceBorder = data->sdlSurfaceBorder;*/
       ...
       /* case SDL_VIDEOEXPOSE:
		...
		case SDL_WINDOWEVENT:
			...
			if (event.window.event == SDL_WINDOWEVENT_RESIZED) {
				...
				rect.x = 0;
				rect.y = 0;
				rect.w = cur_stream->width;
				rect.h = cur_stream->height;
				SDL_RenderSetViewport(cur_stream->sdlRenderer, &rect);*/
								
				//SDL_RenderSetLogicalSize(cur_stream->sdlRenderer, cur_stream->width, cur_stream->height);
				cur_stream->force_refresh = 1;
			}
		...
		case FF_STREAM_OPEN_EVENT:
			...
			if (cur_stream != NULL)
			{
				uint32_t sessionId;
				do {
					sessionId = (uint32_t)our_random32();
			...
			do_stream_open((CustomEventData *)event.user.data1);
			break;
...
static int  do_stream_open(CustomEventData * data)
{
